...
for (const file of commandFiles) {
    ...
    if ("data" in command && "execute" in command) {
        ...
        console.log(`Esse comando em ${filePath} está com "data" ou "execute" ausente(s)`)
    }
}
...
client.once(Events.ClientReady, readyClient => {
	console.log(`O login foi realizado como: ${readyClient.user.tag}`)
});
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    try{
        ...
        console.error(error)
        await interaction.reply("```😕 O comando não existe meu nobre 😕```")
    }
})
